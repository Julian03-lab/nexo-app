...
const Applicant = () => {
  ...
  const { offerId, column, step } = params;
  ...
  const filteredApplicants = applicants[Number(step)].applicantList.filter(
    (app) => app.column === Number(column)
  );

  const generateNewColumnsApplicants = () => {
    const data = filteredApplicants.map((app, index) => {
  ...
  const handlePress = () => {
    setUser({
      ...
      publications: user.publications.map((pub) => {
        if (pub.id === Number(offerId)) {
          return {
            ...pub,
            applicants: applicants.concat({
              stepId: pub.applicants.length + 1,
              applicantList: generateNewColumnsApplicants(),
            }),
          };
        } else {
          return pub;
    ...
    <SafeAreaView style={styles.containter}>
      <ScrollView>
        ...
        <View style={styles.content}>
          ...
          <InputWithLabel label="Link al test">
            <TextInput
              style={styles.input}
              placeholder="Escribe tu link aqui"
            />
          </InputWithLabel>
          <TouchableHighlight
            onPress={handlePress}
